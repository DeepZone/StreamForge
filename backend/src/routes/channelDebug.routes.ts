...
const routes: FastifyPluginAsync = async (app) => {
  app.get('/api/channels/:channelId/twitch/debug', { preHandler: requireAuth }, async (req, rep) => {
    ...
    return { channel: { id: channel.id, twitchLogin: channel.twitchLogin, twitchChannelId: channel.twitchChannelId, isActive: channel.isActive, botEnabled: channel.botEnabled }, session: { exists: Boolean(session), status: session?.status ?? 'missing', eventSubSubscriptions: session?.eventSubSubscriptions ?? null }, liveStream: eventBus.getChannelStats(channelId), tokens: { hasBroadcasterToken: Boolean(token), broadcasterScopes: token ? JSON.parse(token.scopesJson || '[]') : [], hasPlatformBotToken: Boolean(bot?.accessTokenEncrypted), platformBotScopes: bot ? JSON.parse(bot.scopesJson || '[]') : [] }, platformBot: { connected: Boolean(bot), moderatorStatus: botStatus?.status ?? 'unknown', canSendAsPlatformBot: Boolean(botStatus?.isModerator) } };
  });
};
