...
const timersRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.post('/api/channels/:channelId/timers', { preHandler: requireAuth }, async (req: any, rep) => { await requireChannelRole(req, rep, 'channel_moderator'); return prisma.timer.create({ data: { channelId: req.params.channelId, name: req.body.name, message: req.body.message, intervalMinutes: Math.max(1, Number(req.body.intervalMinutes || 1)), enabled: req.body.enabled ?? true } }); });
  app.patch('/api/channels/:channelId/timers/:id', { preHandler: requireAuth }, async (req: any, rep) => { await requireChannelRole(req, rep, 'channel_moderator'); const existing = await prisma.timer.findFirst({ where: { id: req.params.id, channelId: req.params.channelId } }); if (!existing) return rep.code(404).send({ error: 'not_found' }); return prisma.timer.update({ where: { id: req.params.id }, data: { name: req.body.name, message: req.body.message, intervalMinutes: Math.max(1, Number(req.body.intervalMinutes || 1)), enabled: req.body.enabled ?? true } }); });
