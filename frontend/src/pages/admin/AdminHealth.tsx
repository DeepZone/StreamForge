...
export default function AdminHealth() { const { user } = useAuth(); const [data, setData] = useState<any>(null);
  ...
  <button className='rounded-lg bg-indigo-600 px-3 py-2 text-sm' onClick={()=>void apiPost('/api/admin/twitch/eventsub/restart').then(load)}>Restart EventSub</button></div>; }
